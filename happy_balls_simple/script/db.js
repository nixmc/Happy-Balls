...
(function() {
    ...
    request.onsuccess = function(event) {
        ...
        deleteHappinessScore = function(location, opts) {
            var transaction = db.transaction(["happiness"], IDBTransaction.READ_WRITE),
                objectStore = transaction.objectStore("happiness");
            req = objectStore.delete(location);
            req.onsuccess = opts && opts.onSuccess || undefined;
            req.onerror = opts && opts.onError || undefined;
            return req;
        };
        ...
        truncateDB = function() {
            if (confirm("Are you sure?")) {
                var objectStore = db.transaction("happiness").objectStore("happiness");
                objectStore.getAll().onsuccess = function(event) {  
                    var results = event.target.result;
                    results.forEach(function(result) {
                        deleteHappinessScore(result.location);
                    });
                };
            }
        };
